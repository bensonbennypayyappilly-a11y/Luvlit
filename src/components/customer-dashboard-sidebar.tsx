...
const NAV: { to: string; label: string; exact?: boolean }[] = [
  ...
  { to: "/dashboard/requirements", label: "My Requirements" },
  { to: "/dashboard/saved", label: "Saved Businesses" },
...
/** Mirrors BusinessDashboardSidebar so the customer dashboard has the same persistent nav +
 * mobile drawer pattern, instead of each subpage being its own standalone SiteHeader/SiteFooter
 * shell with no shared chrome between them. */
export function CustomerDashboardSidebar({ displayName }: { displayName: string }) {
  ...
  return (
    <>
      ...
      {open && <div className="border-b border-border px-4 py-3 md:hidden">{items}</div>}
